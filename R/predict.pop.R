...
pop.predict <- function(end.year=2100, start.year=1950, present.year=2020, wpp.year=2019,
						...
						mig.is.rate = FALSE, mig.age.method = c("auto", "fdm", "rc", "fdmw"), 
...
load.inputs <- function(inputs, start.year, present.year, end.year, wpp.year, fixed.mx=FALSE, 
                        ...
                        annual = FALSE, mig.age.method = "auto", 
                        verbose=FALSE) {
	observed <- list()
...
migration.totals2age <- function(df, ages = NULL, annual = FALSE, time.periods = NULL, 
    ...
    if(mig.is.rate) {
        ...
        if(method %in% c("fdm", "fdmw")){
            ...
            # and the variance parameter v (for fdmw) 
            ...
            if(method == "fdm") migtmp[, rate_code := 4] else migtmp[, rate_code := if(has.mult.traj) 6 else 5] # using code 5 & 6 for fdmw (6 samples from age-schedules)
        }
...
.get.mig.mx.pasfr.patterns <- function(inputs, wpp.year, pattern.data = NULL, 
    ...
    MIGtype <- create.pattern(vwBase, c('ProjFirstYear', 'MigCode', 'MigFDMb0', 'MigFDMb1', 'MigFDMmin', 'MigFDMsrin', "MigFDMsrout"))
...
.load.mig.traj <- function(inputs, mig.age.method = "fdm", verbose = FALSE) {
