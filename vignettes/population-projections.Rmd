...
We decide on how many trajectories we'd like to generate. The more trajectories, the smoother the results, but the longer the processing time. One can also choose the same number as the national simulation (in our case 1000), obtained via `summary(tfr_nat_pred)`. To keep the processing time low, for this example we choose 50 trajectories. However, this would not be sufficient in a real world simulation.
...
```{r eval=TRUE, include=TRUE, results = FALSE}
e0_pred <- e0.predict.subnat(countries = 840, sim.dir= nat_dir_e0, output.dir = dir_e0,
                             annual = TRUE, end.year = 2050, nr.traj = nr_traj,
...
for (loc in c("Washington", "Mississippi")){
  ...
  e0.trajectories.plot(e0_nat_pred, country = "USA", nr.traj = 0, 
...
```{r eval=TRUE}
e0M_nat_pred <- get.e0.jmale.prediction(e0_nat_pred)
e0.trajectories.table(e0M_nat_pred, "USA")["2021", "median"]
```
