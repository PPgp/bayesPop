...
```{r eval = TRUE, include = TRUE}
data_dir_us <- file.path(data_dir, "bayesPopUSdata-main")
```
...
```
Note that these are big file. Therefore, if you are on a slow network and get a timeout error, you might want to increase the `timeout` option.
...
tfr_pred <- tfr.predict.subnat(countries = 840, sim.dir = world_dir_tfr, output.dir = dir_tfr,
                               annual = TRUE, end.year = 2050, nr.traj = nr_traj_comp, 
...
Here, we are directing the function to generate `r nr_traj_comp`  trajectories of future annual TFR until 2050 for regions found in the file given by argument `my.tfr.file`, that is for regions that belong to country 840 (i.e. the US). Argument `annual` determines that the function expects annual subnational data, as oppose to 5-year data. Argument `sim.dir` points to the national projections, while argument `output.dir` determines where the results are to be stored.
...
If the observed data contain missing values at the end of the time series, these data points are automatically imputed by jumping off the prediction from the latest non-missing time point and using the median of the projection as imputed values. This is the case of American Samoa, where prediction jumped off from 2017. Then the time period from 2018 to 2023 was imputed by using the median of the predictive distribution. These imputed points are shown as green dots in the figure.
...
One can extract all trajectories as a matrix, for example to be used as an input to downstream models:
```{r}
trajs <- get.tfr.trajectories(tfr_pred_us, "Washington")
dim(trajs)
```
...
```{r results = FALSE}
get.country.object(53, country.table = get.countries.table(tfr_pred_us))
get.country.object(48, country.table = get.countries.table(tfr_pred_us), index = TRUE)
```
...
```{r eval=TRUE, include=TRUE}
e0F_subnat_file <- file.path(data_dir_us, "US_states_e0F.txt")
e0M_subnat_file <- file.path(data_dir_us, "US_states_e0M.txt")
read.table(e0F_subnat_file, sep= "\t", header = TRUE, check.names = FALSE) |> head()
```

For each state and territory, the files contain $e_0$ from 2018 through 2021. The meaning of the remaining columns (`reg_code`, `country_code`, `include_code`) is the same as in the case of TFR. Here we don't have any missing values per se, however, since the last observed year for TFR as well as for the national $e_0$ simulation is 2023, we will treat the years 2022 and 2023 as missing $e_0$ for all states and let the simulation to automatically impute it. This will be handled the same way as for TFR, namely by jumping into the simulation from 2021 and replacing the missing 2022 and 2023 values with the projected medians for that year.
...
For analyzing results, various **bayesLife** functions can be used. Here for two states, we view the projected marginal $e_0$ for both sexes, using the national female projections as a background (grey lines) for a comparison:
...
One can see that there are two imputed values for both, the female and male projections, namely for 2022 and 2023. 
...
par(mfrow = c(1,2))
for (loc in c("Washington", "Mississippi"))
  e0.joint.plot(e0_pred_us, loc, years = c(2024, 2035, 2050), 
...
It is an array of time x trajectories. These can be used to create other summaries, or for computing various probabilities. For example, what is the probability that Mississippi's male $e_0$ by 2050 reaches the 2023 national value of 76.9?

```{r eval=TRUE}
sum(trajMiss["2050", ] >= 76.9)/ncol(trajMiss) * 100
```

Note that the 2023 male national $e_0$ was retrieved via
```{r eval=TRUE}
e0M_world_pred <- get.e0.jmale.prediction(e0_world_pred)
e0.trajectories.table(e0M_world_pred, "USA")["2023", "median"]
```
...
We will now use the MCMC results which are stored in `dir_mig` to generate future trajectories of NMR for each state from 2024 to 2050:

```{r eval=simulate_comp, results = FALSE}
mig_pred <- mig.predict(sim.dir = dir_mig, end.year = 2050, 
                        nr.traj = nr_traj_comp, burnin = 1000,
                        save.as.ascii = nr_traj_pop, replace.output = TRUE)
```
...
```{r eval=TRUE}
dir_tfr_us <- file.path(dir_tfr, "subnat/c840")
dir_e0_us <- file.path(dir_e0, "subnat_ar1/c840")
```
...
* Initial population estimates by sex and age (files "US_states_popM.txt" and "US_states_popF.txt").
...
* Historical estimates of sex- and age-specific mortality rates (files "US_states_mxM.txt" and "US_states_mxF.txt").
...
* Historical migration counts (file "US_states_migcounts.txt").
  * Can be given as totals, totals by sex or age- and sex specific counts. We use total counts. The dataset is only used for viewing trends. It does not influence projected migration.
* Historical estimates of sex-ratio at birth.
  * An optional dataset. We will use the national values for all states.
* Dataset of locations (file "US_states_fips.txt").
  *  Regions with "location_type" of 4 will be processed, while the rest will be ignored. The last entry (United States of America) is needed for aggregations.
...
```{r eval=TRUE}
file_locs <- file.path(data_dir_us, "US_states_fips.txt")
```

## Generating Population Projections

Now we generate future population sex- and age-specific trajectories for all US states:

```{r eval=simulate_pop, include=TRUE, results = FALSE}
pop_pred <- pop.predict.subnat(output.dir = dir_pop, 
                locations = file_locs, default.country = 840, 
                annual = TRUE, wpp.year = 2024, 
                present.year = 2023, end.year = 2050,
                nr.traj = nr_traj_pop, verbose = TRUE, 
                inputs = list(
                    popM = file.path(data_dir_us, "US_states_popM.txt"),
                    popF = file.path(data_dir_us, "US_states_popF.txt"),
                    mxM = file.path(data_dir_us, "US_states_mxM.txt"), 
                    mxF = file.path(data_dir_us, "US_states_mxF.txt"),
                    mig = file.path(data_dir_us, "US_states_migcounts.txt"), 
                    migtraj = file_mig_traj,
                    tfr.sim.dir = dir_tfr_us,
                    e0F.sim.dir = dir_e0_us,
                    e0M.sim.dir = "joint_"
                  ),
...
Now we will aggregate over all states.

```{r eval=simulate_pop, include=TRUE, results = FALSE}
pop_aggr <- pop.aggregate.subnat(pop_pred, regions = 840, 
                                 locations = file_locs)
```
...
As in the case of the previous packages, the function `get.countries.table()` work with both, the `pop_pred` and `pop_aggr` objects:
...
Projection results can be viewed either as a function of time or as a function of age. These will be described below. 

### Projections by time

One can plot population projections by time for individual regions (here states) or for the aggregated geography (here the country):

```{r eval = TRUE, fig.width = 6, fig.height = 4.5, fig.align='center'}
pop.trajectories.plot(pop_pred, "Washington", nr.traj = 20)
```
   
```{r eval = TRUE, fig.width = 6, fig.height = 4.5, fig.align='center'}
pop.trajectories.plot(pop_aggr, 840, nr.traj = 20)
``` 

Tabular results can be viewed for example via

```{r, eval = TRUE}
pop.trajectories.table(pop_pred, "Washington") |> head()
```

### Projections by age

To view projections as a function of age, do for example

```{r eval = TRUE, fig.width = 6, fig.height = 4.5, fig.align='center'}
pop.byage.plot(pop_pred, "Texas", year = 2050, nr.traj = 20)
```
...
```{r eval = TRUE, fig.width = 6, fig.height = 4.5, fig.align='center'}
pop.pyramid(pop_pred, "Nebraska", year = c(2050, 2023), proportion = TRUE)
```
