...
We decide on how many trajectories we'd like to generate. The more trajectories, the smoother the results, but the longer the processing time. One can also choose the same number as the national simulation (in our case 1000), obtained via `summary(tfr_nat_pred)`.
...
tfr_pred <- tfr.predict.subnat(countries = 840, sim.dir = nat_dir_tfr, output.dir = dir_tfr,
                               annual = TRUE, end.year = 2050, nr.traj = nr.traj, 
                               verbose = TRUE, my.tfr.file = tfr_subnat_file
                               )
```

Here, we are directing the function to generate `r nr.traj`  trajectories of future annual TFR until 2050 for all regions found in the file given by argument `my.tfr.dir` that belong to country 840 (i.e. the US), expecting annual subnational data. Argument `sim.dir` points to the national projections, while argument `output.dir` determines where the results are stored.
...
The directory pointing to the national $e_0$ projections is 

```{r eval = TRUE, include = TRUE}
nat_dir_e0 <- file.path(data_dir, "e01", "sim20230204")
```

To explore the US projections one can use various functions from the **bayesLife** package. For example:

```{r eval = TRUE, include = TRUE, fig.width = 7, fig.height = 5, fig.align='center'}
e0_nat_pred <- get.e0.prediction(nat_dir_e0)
e0.trajectories.plot(e0_nat_pred, country = "USA", nr.traj = 50, both.sexes = TRUE)
```
