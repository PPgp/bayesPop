...
dim(trajs)
```
The dimensions of the resulting matrix correspond to the number of time points (`r nrow(trajs)`) x number of trajectories (`r nr_traj`).
...
mig_mcmc <- run.mig.mcmc(nr.chains = mig_nr_chains, iter = mig_iter, 
                         ...
                         annual = TRUE, present.year = 2024, pop.denom = 1000)
```
...
An optional argument `start.year` could be used to limit the time span of the observed data used for the estimation. Here we use all available data from 2010 to 2024.
...
We will now use the MCMC results which are stored in `dir_mig` to generate future trajectories of NMR for each state from 2025 to 2050:

```{r eval=TRUE, results = FALSE}
mig_pred <- mig.predict(sim.dir = dir_mig, end.year = 2050, 
                        nr.traj = nr_traj, burnin = 100,
                        save.as.ascii = nr_traj, replace.output = TRUE)
```
...
Now we generate future population trajectories for all US states:

```{r eval=TRUE, include=TRUE, results = FALSE}
pop_pred <- pop.predict.subnat(output.dir = dir_pop, default.country = 840, 
                locations = file.path(data_dir_us, "US_states_fips.txt"),
                annual = TRUE, wpp.year = 2024, 
                present.year = 2023, end.year = 2050,
                nr.traj = nr_traj, verbose = TRUE, 
                inputs = list(
...
```
Note that by default total migration is distributed into ages using a basic Rogers-Castro function. An alternative method is implemented in **bayesPop**, the Flow Difference Method (Ševčíková, Raymer and Raftery, 2024), which might be more suitable for subnational units that experience a different pattern than Rogers-Castro, for example, regions with high migration of retirees.
    
Also note that this is a toy simulation. Normally we would want to generate 1000 or more trajectories.
